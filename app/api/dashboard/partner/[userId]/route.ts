 ...
 * 
 * CACHING: 30-second cache for real-time updates with performance
 */
export async function GET(
  ...
  try {
    ...
    // Use dashboard cache with 30-second TTL for real-time + performance balance
    const dashboardData = await CacheHelpers.dashboard.getPartnerData(
      params.userId,
      async () => {
    ...
    // PRODUCTION CACHE HEADERS - Balance between real-time and performance
    // Cache for 30 seconds with stale-while-revalidate for instant loading
    response.headers.set('Cache-Control', 'public, max-age=30, stale-while-revalidate=15')
    response.headers.set('X-Cache-Status', 'dashboard-optimized')
